...
namespace Goodreads.API.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        [ProducesResponseType(typeof(ApiResponse), StatusCodes.Status200OK)]
        [ProducesResponseType(typeof(ApiResponse), StatusCodes.Status400BadRequest)]
        public async Task<IActionResult> ResetPassword([FromQuery] string userId, [FromQuery] string token, [FromBody] Application.Auth.Commands.ResetPassword.ResetPasswordRequest request)
        {
            var result = await mediator.Send(new ResetPasswordCommand(userId, token, request.NewPassword));
            return result.Match(
